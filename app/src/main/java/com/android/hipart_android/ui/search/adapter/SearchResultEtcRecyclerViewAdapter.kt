...
class SearchResultEtcRecyclerViewAdapter(val ctx: Context, val dataList: ArrayList<User>) :
    RecyclerView.Adapter<SearchResultEtcRecyclerViewAdapter.Holder>() {
    ...
    override fun onBindViewHolder(holder: Holder, position: Int) {
        ...
        if (dataList[position].info[0].etc != 0) {
            holder.pd.text = Filter.etc(dataList[position].info[0].etc)
        } else {
